/*
 * holdering_registers.c
 *
 *  Created on: Aug 11, 2025
 *      Author: SUser
 */
#include "modbus/modbus_command_support/holding_registers.h"
#define get_holding_register_value(register)				0xAAAA
...
modbus_request_status_code_t read_holding_register_handler(uint8_t *data, uint8_t len){
	...
	if(cmd->register_number >= 	HOLDING_REGISTER_COUNTER){
		return INVALID_DATA_ADDRESS;
	}
	if (cmd->register_counter > (HOLDING_REGISTER_COUNTER - cmd->register_number)) {
	...
	tx_buffer[tx_len++] = (uint8_t)READ_HOLDING_REGISTERS;
	...
	for(uint16_t i = 0; i < cmd->register_counter; i++){
		...
		value.u16 = get_holding_register_value(cmd->register_number + i);
